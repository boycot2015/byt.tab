...
const getWallpaper = async (
  params: Record<string, any> = { source: '', id: '0' }
) => {
  ...
  const response = await $GET(
    `${urlMap[params.source || '0'].url}${urlMap[params.source || '0'].searchKey}=${params?.id || ''}&page=${params?.page || 1}&size=${params?.size || 12}`
  )
  ...
  return {
    ...
    list:
      [...(data?.list || data)]?.map((el) => ({
        ...el,
        img: el.thumb || el.img || '',
        url: el.fullSrc || el.raw || el.url || '',
        id: el.id || el._id
      })) || []
  }
}
const getWallpaperCategory = async (
  ...
  if (urlMap[params.source || '0']?.url.includes(codelifeUrl)) {
    ...
    let category = await $GET(
      `${[codelifeUrl, params.source].join('/')}/category?lang=cn`
    )
    if (params.source === 'wallpaper')
      category.data?.unshift({ id: '0', name: '推荐壁纸' })
    console.log(category.data, '$GET-推荐壁纸')
    cates = [...(category.data || [])]
  }
  if (urlMap[params.source || '0']?.url.includes(baseUrl)) {
    let category2 = await $GET(
      `${baseUrl}/wallpaper?lang=cn&source=${params.source || ''}`
    )
    // console.log(category2.data, '$GET-birdpaper')
    cates = [
      ...(params.source && !params.type
        ? category2.data?.cates || []
        : category2.data?.source || [])
    ]
  }
  if (cates?.length === 0 && params.source == 'wallpaper') {
    let category = await getWallpaperCategory({ source: '0', type: 'source' })
    let category2 = await getWallpaperCategory({
      source: 'birdpaper',
      type: 'source'
    })
    cates = [...(category || baseCates), ...(category2 || [])].filter(
      (el, index, self) => self.findIndex((item) => item.id === el.id) === index
    )
  }
  return (
    cates?.map((el) => {
